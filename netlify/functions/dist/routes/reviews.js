...
const ReviewSubmissionSchema = zod_1.z.object({
    ...
    content: zod_1.z.string().min(10, 'Review must be at least 10 characters').max(2000, 'Review must be 2000 characters or less'),
    photos: zod_1.z.array(zod_1.z.object({
        storagePath: zod_1.z.string(),
        publicUrl: zod_1.z.string().url(),
...
const ReviewFiltersSchema = zod_1.z.object({
    page: zod_1.z.string().transform((val) => parseInt(val) || 1).optional(),
    limit: zod_1.z.string().transform((val) => Math.min(parseInt(val) || 10, 50)).optional(),
    rating: zod_1.z.string().transform((val) => parseInt(val)).optional(),
    verified: zod_1.z.string().transform((val) => val === 'true').optional(),
    withPhotos: zod_1.z.string().transform((val) => val === 'true').optional(),
    sortBy: zod_1.z.enum(['newest', 'oldest', 'rating-high', 'rating-low', 'helpful']).optional()
});
...
router.post('/', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (reviewData.photos && reviewData.photos.length > 0) {
            const photoData = reviewData.photos.map((photo) => ({
                reviewId: review.id,
...
router.post('/:reviewId/helpful', auth_1.authenticateToken, async (req, res) => {
    try {
    ...
    try {
        ...
        if (updateData.photos !== undefined) {
            ...
            if (updateData.photos.length > 0) {
                const photoData = updateData.photos.map((photo) => ({
                    reviewId,
                    storagePath: photo.storagePath,
                    publicUrl: photo.publicUrl,
